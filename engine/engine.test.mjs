...
test("каталог подставляет данные арендатора в тексты ответов", async () => {
  const assistant = createAssistant(base());
  assert.match((await assistant.ask("Какой телефон?")).text, /\+7 000 000-00-00/);
  assert.match((await assistant.ask("Я больше не могу так жить")).text, /112/);
});

test("кризис проверяется раньше любых правил арендатора", async () => {
  ...
  const answer = await assistant.ask("Я больше не могу так жить");
  assert.equal(answer.kind, "crisis");
});

test("кризисный ответ не предлагает навигацию, даже если каталог её просит", async () => {
  const assistant = createAssistant(withCatalog((catalog) => {
    catalog.pipeline[0] = { safety: "crisis", sources: ["home", "help"] };
  }));
  assert.deepEqual((await assistant.ask("У меня мысли о суициде")).sourceKeys, []);
});

test("каталог без кризиса первым шагом не запускается", async () => {
...
test("каталог без обязательного шага безопасности не запускается", async () => {
  assert.throws(() => prepareCatalog(withCatalog((catalog) => {
    catalog.pipeline = catalog.pipeline.filter((step) => step.safety !== "clinical");
  })), /обязательного шага безопасности «clinical»/);
});

test("шаг безопасности нельзя переопределить регулярным выражением", async () => {
  assert.throws(() => prepareCatalog(withCatalog((catalog) => {
    catalog.pipeline[3] = { safety: "clinical", match: "ничего", sources: ["help"] };
  })), /не настраивается регулярным выражением/);
});

test("источник по незащищённому адресу не принимается", async () => {
  assert.throws(() => prepareCatalog(withCatalog((catalog) => {
    catalog.sources.home.url = "http://example.org/";
  })), /не на https-адрес/);
});

test("ссылка на несуществующий ответ или источник ловится до первого вопроса", async () => {
  assert.throws(() => prepareCatalog(withCatalog((catalog) => {
...
test("пустой вопрос и незнакомый вопрос ведут себя как описано в каталоге", async () => {
  const assistant = createAssistant(base());
  assert.equal((await assistant.ask("   ")).kind, "route");
  const unknown = await assistant.ask("Какая завтра погода?");
  assert.equal(unknown.kind, "unknown");
  assert.deepEqual(unknown.sourceKeys, ["home"]);
});

test("страницы арендатора могут быть путями его собственного сайта", async () => {
  const assistant = createAssistant(base());
  assert.equal(assistant.sources.help.url, "/consultation");
  assert.equal((await assistant.ask("Посоветуйте антидепрессант")).sourceKeys[0], "help");
});
...
test("неподтверждённая запись FAQ помогает найти страницу, но не произносится", async () => {
  const assistant = createAssistant(withFaq(false));
  const answer = await assistant.ask("Принимаете подростков?");
...
test("подтверждённая запись FAQ становится ответом", async () => {
...
test("запись FAQ на несуществующий источник не запускается", async () => {
  assert.throws(() => prepareCatalog(withCatalog((catalog) => {
...
test("подтверждение без следа не принимается", async () => {
